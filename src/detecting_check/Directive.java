...
public abstract class Directive {
	...
	public boolean isInCheck(int isDark, ChessBoard chessBoard, Piece[] darkPieces, Piece[] lightPieces)
	{
		boolean valid = false;
		King king = null;
		king = (King) findPiece('k', isDark, chessBoard, lightPieces, lightPieces);
		
		if(isDark == 1)
		{
			for(int i = 0; i < lightPieces.length; i++)
			{
				if(lightPieces[i].captureIsValid(new Position(king.getPosition().getRow(), king.getPosition().getColumn()), chessBoard, darkPieces, lightPieces))
				{
					valid = true;
				}
			}
		...
		else
		{
			for(int i = 0; i < darkPieces.length; i++)
			{
				if(darkPieces[i].captureIsValid(new Position(king.getPosition().getRow(), king.getPosition().getColumn()), chessBoard, darkPieces, lightPieces))
				{
					valid = true;
				}
			}		
